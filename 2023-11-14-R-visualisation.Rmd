...
[`https://github.com/ELIXIREstonia/2023-11-14-R-visualisation`](https://github.com/ELIXIREstonia/2023-11-14-R-visualisation).
...
R has several data structures for organizing and storing data. Understanding
these data structures is essential for effective data manipulation and analysis.

1.  **Vectors**: A one-dimensional array of elements, all of the same data type.
    Vectors are commonly used for performing arithmetic operations and storing
    sequences of values.
...
2.  **Matrices**: A two-dimensional array of elements, organized in rows and
    columns, and all of the same data type. Matrices are useful for linear
    algebra operations and storing tabular data with a fixed number of rows and
    columns.
...
3.  **Data frames**: A two-dimensional table where each column can have a
    different data type. Data frames are the most common data structure for
    storing and manipulating tabular data in R, especially for statistical
    analysis.
...
4.  **Lists**: A versatile data structure that can store elements of different
    data types and structures, including vectors, matrices, data frames and
    other lists. Lists are useful for organizing and storing hierarchical or
    nested data.
...
Each data structure has its strengths and applications, and choosing the right
one depends on the specific needs of your data manipulation and analysis tasks.

### Data Types in R

Understanding the fundamental data types in R is crucial for effective
programming and data manipulation. Here's an overview of the primary data types
you'll encounter in R:

1.  **Numeric**: This is the default data type for numbers in R. Numeric types
    in R are essentially decimal (floating-point) numbers. They are used for
    most calculations involving numbers.
...
2.  **Integer**: Integers are numbers without decimal points. They can be
    explicitly defined by adding L to the end of the number.
...
3.  **Character**: This type represents strings in R. Character strings are
...
4.  **Logical**: This type, also known as Boolean, represents TRUE or FALSE
    values and is often the result of comparisons and logical operations.
...
5.  **Factor**: Factors are used to handle categorical data and store it as
    levels. They can be ordered or unordered and are particularly useful in
...
6.  **Complex**: This data type is used for complex numbers with real and
    imaginary parts.
...
7.  **Raw**: The raw type is used to handle raw bytes. It's less commonly used
    than the other data types but is important for certain types of binary data
    operations.
...
Each data type is suited for different kinds of tasks in R, and understanding
these types is foundational for data analysis and programming in this
environment. Choosing the correct data type is key to efficient programming and
...
For the Excel file, you'll need to remember to specify the correct sheet name
using the `sheet` parameter in the `read_excel()` function.
...
1.  **Data Exploration**:
    ...
    -   Calculate the overall average of memory scores before and after the drug
        was administered.

2.  **Drug Analysis**:

    -   Using tidyverse pipes and the provided example as a reference, group the
        data by the **`Drug`** column.
...
3.  **Dosage Analysis**:
    ...
    -   For each drug and dosage combination, calculate the average **`diff`**
        in memory scores.

    -   Display the results, sorting first by **`Drug`** and then by the average
        **`diff`** in descending order.
...
4.  **Age Group Analysis**:
    ...
    -   Calculate the average **`diff`** in memory scores for each age group and
        drug combination.

    -   Display the results, and try to infer if age has any impact on the
...
5.  **Happy/Sad Group Analysis**:
    ...
    -   For each emotion group, calculate the average **`diff`** in memory
        scores before and after drug administration.

    -   Compare the results. Does the primed emotion seem to have an effect on
        the memory scores?
